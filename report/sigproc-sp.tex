...
The two main attributes that a news has are title and body. However, we noticed that in many cases considering only the title for the clustering part is more convenient because the title is a collection of few words, tipically keywords, that summarize the entire news.
...
Considering a comparision between the tables above, the difference between the internal and the external similarity is the same for both the tecniques. However, the time spent for the whole process which uses the set of words to build the characteristic matrix is definitively lower than the process which uses a matrix build by the set of shingles. As conclusion, we decided to use a characteristic matrix build starting from the set of words.
...
The third test we did was to determine the quality of the clusters using only the title or both the title and the body as information of the news.
