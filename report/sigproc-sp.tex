...
\maketitle
\begin{abstract}
Every day lots of articles are published by many newspapers and websites. As a consequence, the same news is repeated in many sources causing confusion to readers which have to choose the most interesting ones. Fortunately, there are websites which everyday get articles from different surces and group them by removing duplicates. The most famous one is Google News\footnote{https://news.google.com}. It actually groups sources which treat the same news and gives to readers a list of the most representative articles for each group.




While the title provided by Google is the same as the one written in the source, the body is a preview of the article which belongs to the source and which is a paragraph that allows to readers to get instantaneously significant information. The aims of our work is to replicate what Google News provides, focusing on italian articles.
\end{abstract}
